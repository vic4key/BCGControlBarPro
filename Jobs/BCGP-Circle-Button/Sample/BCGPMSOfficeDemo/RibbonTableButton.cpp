...
CRibbonTableButton::CRibbonTableButton(
	...
	CBCGPRibbonPaletteButton (nID, lpszText, nSmallImageIndex, nLargeImageIndex,
							globalUtils.ScaleByDPI(CSize(20, 20)), 0, FALSE)
{
											...
											COLORREF /*clrText*/)
{
	...
	COLORREF color = globalData.clrBtnDkShadow;
	BOOL bHighlighted = (nIconIndex / CELLS_X + 1 <= m_nRows && (nIconIndex % CELLS_X) + 1 <= m_nColumns);
	...
	int nDelta = bHighlighted ? 2 : 1;
	rectIcon.DeflateRect(nDelta, nDelta);
